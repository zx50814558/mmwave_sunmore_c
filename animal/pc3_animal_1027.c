		  ...
		  if (state == 5)
		  {
			  ...
			  if (frame_number_inf>2) //前3frame會繼續累加點雲
			  {
				  ...
				  */
				  ...
				  int zero_count = 0;
				  printf("dbscan_point\n");
				  for(int num=1; num < wo_nan-1; ++num)
				  {
					if (dbscan_point[num] ==0)
					{
						zero_count+=1;
					}
					if (dbscan_point[num-1] == 0 && dbscan_point[num] > 0)
					{
						printf("抓到 %d  %d 數0 %d\n", num, dbscan_point[num], zero_count);
						for(int num_1=0; num_1 < wo_nan; ++num_1)
						{
							if (index_point[num_1] ==  (float) num)
							{
								index_point[num_1] = (float) num-zero_count;
							}
						}
					}
					else
